...
- **Types** schema panel (PR05/PR12) — loads `public/demo-types/` + `public/demo-schema/` (`?panel=types`); generate with `./scripts/demo-types.sh`
...
- CLI proof: `./scripts/demo-vault.sh` / `./scripts/demo-schema.sh` / `./scripts/demo-markdown.sh` / `./scripts/demo-index.sh` / `./scripts/demo-objects.sh` / `./scripts/demo-editor.sh` / `./scripts/demo-daily.sh` / `./scripts/demo-created-today.sh` / `./scripts/demo-types.sh`
